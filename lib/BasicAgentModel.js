...
// a customer enters the canteen UNTREATED; a customer then is QUEUEING to be treated by a doctor; 
...
function updateSurface(){
	...
	newcaregivers.append("svg:image")
	 ...
	 .attr("width", Math.min(cellWidth,cellHeight)+"px")
	 .attr("height", Math.min(cellWidth,cellHeight)+"px")
	 .attr("xlink:href",function(d){if (d.type==DOCTOR) return urlCashier; else return urlReceptionist;});
  ...
  function updatePatient(patientIndex) {
	...
	case TREATED:
		if (hasArrived){
			...
			console.log(patient.location.col, " getCurrentCafe(patient)");
...
function updateDynamicAgents(){
	...
	setInterval(() => {
		...
		const data = {
			currentTime,
			patientsTreated: statistics[0].count,
			patientsDischarged: statistics[1].count,
			patientsExited: statistics[2].count,
			...
			totalPatients: statistics[13].count
		  };
