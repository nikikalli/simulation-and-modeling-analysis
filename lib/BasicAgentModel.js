...
var currentTime = 0;
var statistics = [
{"name":"Average time in clinic, Type A: ","location":{"row":workerRow+13,"col":workerCol-4},"cumulativeValue":0,"count":0},
// {"name":"Average time in clinic, Type B: ","location":{"row":workerRow+14,"col":workerCol-4},"cumulativeValue":0,"count":0},
{"name":"Reject percentage ","location":{"row":workerRow+15,"col":workerCol-4},"cumulativeValue":0,"count":0}
];
	...
	switch (state) {
	  case UNTREATED:
		...
		let randomIndex = Math.floor(Math.random() * queues.length);
			let randomCafe = cafes[randomIndex];
	...
	switch(state){
		case UNTREATED:
			...
			if (hasArrived) {
				...
				patient.state = randomCafe.state;
				patient.target.row = chosenQueue[amoutOfFreeSpotsIntheQueue - 1].row;
				patient.target.col = chosenQueue[amoutOfFreeSpotsIntheQueue - 1].column;
				chosenQueue[amoutOfFreeSpotsIntheQueue - 1].state = OCCUPIED;
		...
		case CAFE1:
				...
				var cafeIndex = state - CAFE1;
				var cafeQueue = queues[cafeIndex];

				var currentQueueIndex = cafeQueue.findIndex(n => n.row === patient.location.row && n.column === patient.location.col);
				...
				if (currentQueueIndex === cafeQueue.length - 1 && hasArrived) {
					...
					let patientInStaging = patients.find(p => p.location.row === staging.row && p.location.col === staging.col);

					if (!patientInStaging && !isOnTheWay[0].state) {
					isOnTheWay[0].state = true
					cafeQueue[currentQueueIndex].state = EMPTY;
					patient.state = STAGING;
					patient.target.row = staging.row;
					patient.target.col = staging.col;
					}
				} else if (hasArrived && nextQueueIndex < cafeQueue.length && cafeQueue[nextQueueIndex].state === EMPTY) {
		...
		case CAFE2:
				...
				if (currentQueueIndex === cafeQueue.length - 1 && hasArrived) {
					...
					if (!patientInStaging && !isOnTheWay[1].state) {
					isOnTheWay[1].state = true
					cafeQueue[currentQueueIndex].state = EMPTY;
					patient.state = STAGING;
					patient.target.row = staging.row;
					patient.target.col = staging.col;
		...
		case CAFE3:
			...
			if (currentQueueIndex === cafeQueue.length - 1 && hasArrived) {
				...
				if (!patientInStaging && !isOnTheWay[2].state) {
				isOnTheWay[2].state = true
				cafeQueue[currentQueueIndex].state = EMPTY;
				patient.state = STAGING;
				patient.target.row = staging.row;
				patient.target.col = staging.col;
		...
		case CAFE4:						
				...
				if (currentQueueIndex === cafeQueue.length - 1 && hasArrived) {
					...
					if (!patientInStaging && !isOnTheWay[3].state) {
					isOnTheWay[3].state = true
					cafeQueue[currentQueueIndex].state = EMPTY;
					patient.state = STAGING;
					patient.target.row = staging.row;
					patient.target.col = staging.col;
		...
		case CAFE5:
			...
			if (!patientInStaging && !isOnTheWay[4].state) {
				isOnTheWay[4].state = true
				cafeQueue[currentQueueIndex].state = EMPTY;
				patient.state = STAGING;
				patient.target.row = staging.row;
				patient.target.col = staging.col;
		...
		case STAGING:
			...
			if (hasArrived){
				...
				if (doctor.state == IDLE){
					...
					doctor.state = BUSY;
					patient.state = INTREATMENT;
					// patient.target.row = workerRow + 4;
					// patient.target.col = workerCol;
		...
		case TREATED:
			if (hasArrived){
				console.log("hello")
				console.log(isOnTheWay[doctor.label - 2], " before")
				isOnTheWay[doctor.label - 2].state = false
				console.log(isOnTheWay[doctor.label - 2], " after")
				patient.state = DISCHARGED;
				patient.target.row = 1;
				patient.target.col = maxCols;
