...
var nextTreatedPatientID_A =1; //this is the id of the next patient of type A to be treated by the cashier
// var nextTreatedPatientID_B =1; //this is the id of the next patient of type B to be treated by the cashier
...
function redrawWindow(){
	...
	nextTreatedPatientID_A =1; //this is the id of the next patient of type A to be treated by the cashier
	// nextTreatedPatientID_B =1; //this is the id of the next patient of type B to be treated by the cashier
	currentTime = 0;
	cashier.state=IDLE;
...
function processCafeState(patient, state, hasArrived) {
	...
	if (currentQueueIndex === cafeQueue.length - 1 && hasArrived) {
	  ...
	  const patientInStaging = customers.find(
...
/*const setCafeState = (cafeIndex, queue) => {
		patient.timeAdmitted = currentTime;
		patient.state = CAFE1 + cafeIndex;
		const lastIndex = queue.filter(n => n.state == EMPTY).length - 1;
		patient.target = { row: queue[lastIndex].row, col: queue[lastIndex].column };
		queue[lastIndex].state = OCCUPIED;
	  };

	const chooseCafe = (indices) => {
		for (const index of indices) {
		  const queue = queues[index], freeSpots = queue.filter(n => n.state == EMPTY).length;
		  if (freeSpots > 0) { setCafeState(index, queue); return; }
		}
		patient.state = DISCHARGED;
		patient.target = { row: 1, col: maxCols };
	  };*/
...
function fisherYatesShuffle(arr) {
  ...
  function updatePatient(patientIndex) {
	//patientIndex is an index into the customers data array
	...
	switch (state) {
	  case UNTREATED:
		...
		if (hasArrived) {
			...
			if (selectedOption) {
				patient.timeAdmitted = currentTime;
				patient.state = CAFE1 + selectedOption.option;
				patient.target = { row: selectedOption.queue[selectedOption.spots - 1].row, col: selectedOption.queue[selectedOption.spots - 1].column };
				selectedOption.queue[selectedOption.spots - 1].state = OCCUPIED;
				patient.id = ++nextPatientID_A;

				statistics[13].count++;

				// console.log(statistics[12], " moro")
	  ...
	  case CAFE6:
		...
		case STAGING:
			if (hasArrived) {
				const currentCashier = caregivers.find(cg => cg.location.row === patient.location.row + 1 && cg.location.col === patient.location.col);
